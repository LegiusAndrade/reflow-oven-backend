# DONE — Backend (histórico do que já foi entregue)
...
- ✅ **#5 — Limpeza de banco: categorias + permissões.** Overview devolve `programas`/`usuarios` (exceto o
  logado, server-side) com bytes reais. **Lixeira `programas_deletados`/`usuarios_deletados` é Master-only**
  (Admin nem vê no overview e leva 403); dados vivos (ativos/inativos/salvos) são **Admin-only**; histórico
  (execuções/falhas/logs) limpável por Admin **e** Master.
- ✅ **#10 — `CreatedAt` em `ReflowProgram`** + sort "Padrão"/"Recente" por `LastUsed ?? CreatedAt` (recém-criado
  abre em 1º). Migration `AddProgramCreatedAt`.
- ✅ **#11 — dev Master fora das contagens.** `DiagnosticsService` exclui o Master de
  `activeUsers`/`inactiveUsers`/`admins` e do ranking `topUsers`; idem `SystemService` (`/api/system/audit`).
- ✅ **#12 — `CreatedById` em Programa e Usuário.** O Master deleta (item a item) só o que criou; flag
  `canDelete` por viewer no DTO; o serviço impõe a posse. Migration `AddCreatedBy`. Removida a policy
  `AdminStrict` (user `DELETE` volta a `AdminOnly`).
- ✅ **Auto-delete bloqueado** — `UserService.DeleteAsync` rejeita apagar a própria conta (403).
- ✅ **Restaurar usuário volta sempre `Inativo`** (nunca reativa sozinho).
- ✅ **dev Master escondido das Alterações** — `AuditService` não grava ChangeLog nem contadores de atividade
  para o Master (só o Log de Operação Master-only); o demo-seed não atribui execuções/erros/alterações ao Master.
- ✅ **#13 — Regular inicia/para execução** (`OperatorOrAdmin`) — verificado: `vanessa` (Regular) → HTTP 200.
- ✅ **#14 — cadastro MODELO B** — senha gerada + enviada por e-mail; o login com ela funciona (verificado).
  Decidido manter MODELO B.

## Lote 2026-06-01 → 06-04

- ✅ **#1 — Seed limpo em produção** (usuários/dados demo só em dev).
- ✅ **#2 — Notificações em tempo real** (`/hubs/notifications`, espelhando o `SystemLogHub`).
- ✅ **#4 — Limpeza real** confirmada (`ExecuteDelete`, não mock).
- ✅ **#6 — `osKernel` real** (uname -r) no overview da Manutenção.
- ✅ **#8 — 2 casas decimais** nos valores de programa/telemetria (config/PID/calibração de fora).
- ✅ **#9 — Log de Operação (backend)** — trilha única append-only `OperationLog`, Master-only, com filtros
  (`category`/`operator`/`type`/`object`/`objectId` + paginação). Falta só a **tela do front**.
