...
/// enter a profile point below it, EXCEPT the fixed t=0 ambient start (<see cref="DomainConstants.StartTemp"/>
/// = 25 °C), which the editor prepends and the import path accepts. Runs against the EF Core InMemory
...
public sealed class ProgramServiceTempFloorTests
{
    ...
    [Fact]
    public async Task CreateAsync_accepts_a_segment_at_the_temp_floor_and_keeps_the_ambient_start()
    {
        var service = NewService(NewContext());
        ...
        // The derived curve keeps the prepended ambient start (25 °C, below the floor by design)…
    ...
    [Fact]
    public async Task CreateAsync_accepts_an_imported_profile_that_starts_at_ambient()
    {
        var service = NewService(NewContext());
        // The t=0 ambient start is below the floor but exempt; every later point is at/above it.
