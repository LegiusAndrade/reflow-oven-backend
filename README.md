...
| `42501: permission denied to create database` | O banco `reflowoven` não existe e o papel `reflow` **não tem `CREATEDB`** (proposital — menor privilégio: o app não cria bancos). | Recrie via superusuário: `sudo -u postgres psql -c "CREATE DATABASE reflowoven OWNER reflow;"` (ou, **só em dev**, dê o direito ao papel: `ALTER ROLE reflow CREATEDB;`), depois `dotnet run`. |
